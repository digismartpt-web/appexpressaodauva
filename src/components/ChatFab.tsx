...
export default function ChatFab({ onOpen, isOpen }: ChatFabProps) {
  ...
  return (
    <>
      {showPulse && (
        <div className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-[#111]/5 animate-ping pointer-events-none z-50" />
      )}
      <button
        onClick={onOpen}
        className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-[#111] text-white shadow-xl hover:shadow-2xl hover:scale-105 active:scale-95 transition-all z-50 flex items-center justify-center border border-[#C8A96E]/30"
